...
/** KPI ribbon, charts, and side panels — shown while scope changes or the home page loads. */
export function DashboardHomeBodySkeleton() {
  return (
        ...
        <Card className="rounded-2xl border-border/60 xl:col-span-2">
          <CardHeader className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <Shimmer key={i} className="h-8 w-14 rounded-md" />
              ))}
            </div>
          </CardHeader>
...
/** Full home route loading state (scope header + body). */
